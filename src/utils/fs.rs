...
/// symlink attacks.
pub fn lock_file_with_permissions(path: &Path) -> io::Result<fd_lock::RwLock<std::fs::File>> {
...
pub fn lock_exclusive_with_timeout<F>(
    lock: &mut fd_lock::RwLock<std::fs::File>,
    timeout: Duration,
    on_lock: F,
) -> io::Result<()>
where
    F: FnOnce(fd_lock::RwLockWriteGuard<'_, std::fs::File>),
{
