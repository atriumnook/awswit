# awswit shell integration for Zsh.
...
_awswit_variable_safe() {
    local attributes=${parameters[$1]-}
    [[ -z "$attributes" || "$attributes" = scalar || "$attributes" = scalar-export ]]
}
...
awswit() {
    ...
    if [[ "$#" -eq 0 ]]; then
        if output=$(AWSWIT_HOOK=1 AWSWIT_SHELL=zsh command awswit activate); then :; else
            exit_code=$?
            return "$exit_code"
        fi
    else
        case "$1" in
            activate|unset)
                if output=$(AWSWIT_HOOK=1 AWSWIT_SHELL=zsh command awswit "$@"); then :; else
                    exit_code=$?
                    return "$exit_code"
                fi
                ;;
            doctor)
                AWSWIT_HOOK=1 AWSWIT_SHELL=zsh command awswit "$@"
                return $?
                ;;
...
if (( $+functions[compdef] )); then
    _awswit_with_profiles() {
        local first profile root_position=0 profile_position=0
        local -a profiles
        first=${words[2]-}

        if (( CURRENT == 2 )); then
            root_position=1
        elif (( CURRENT == 3 )) && [[ "$first" = activate || "$first" = exec ]]; then
            profile_position=1
        fi

        _awswit "$@"

        if (( ! root_position && ! profile_position )); then
            return 0
        fi

        profiles=("${(@f)$(command awswit list --format completion 2>/dev/null)}")
        for profile in "${profiles[@]}"; do
            ...
            if [[ "$profile" = -* ]]; then
                compadd -- "--profile=$profile"
            else
                compadd -- "$profile"
            fi
        done
    }
    compdef _awswit_with_profiles awswit
fi
